...
class LoggingMiddleware(BaseMiddleware):
    ...
    async def on_post_process_callback_query(self, callback_query: CallbackQuery, results: list, data: dict) -> None:
        if callback_query.data in callback_dict:
            ...
            try:
                text = f"{hash_user_id[54:]}|{callback_dict[callback_query.data]}|{str(callback_query.message.text[:96])}...|{SectionName.s_name}"
            except TypeError:
                text = f"{hash_user_id[54:]}|{callback_dict[callback_query.data]}|Без сообщения|{SectionName.s_name}"
